...
// PRODUKTE – D1
// ============================================================
function productDto(row){let variants=[],tags=[];try{variants=JSON.parse(row.variants_json||'[]')}catch{}try{tags=JSON.parse(row.tags_json||'[]')}catch{}return{id:row.product_id,sku:row.sku||'',name:row.name,description:row.description||'',category:row.category||'',price_cents:row.price_cents,price:row.price_cents/100,variants,tags,personalization_label:row.personalization_label||'',personalization_placeholder:row.personalization_placeholder||'',stock:row.stock,unlimited_stock:!!row.unlimited_stock,active:!!row.active,sort_order:row.sort_order||0}}
...
function normalizeProduct(b){const name=clean(b.name,120),sku=clean(b.sku,60),description=clean(b.description,1000),category=clean(b.category,100),label=clean(b.personalization_label,80),placeholder=clean(b.personalization_placeholder,120),price=Number.parseInt(b.price_cents,10),stock=Number.parseInt(b.stock,10),sort=Number.parseInt(b.sort_order,10)||0,variants=Array.isArray(b.variants)?b.variants.map(v=>clean(v,80)).filter(Boolean).slice(0,30):[],tags=Array.isArray(b.tags)?b.tags.map(v=>clean(v,50)).filter(Boolean).slice(0,20):[];if(!name||!Number.isInteger(price)||price<0||price>10000000)throw Error('Name oder Preis ungültig.');if(!b.unlimited_stock&&(!Number.isInteger(stock)||stock<0||stock>1000000))throw Error('Bestand ungültig.');return{name,sku,description,category,label,placeholder,price,stock:b.unlimited_stock?0:stock,sort,variants,tags,unlimited:b.unlimited_stock?1:0,active:b.active===false?0:1}}
async function createAdminProduct(request,env,origin){const a=await authenticateAdmin(request,env);if(!a.ok)return reply({ok:false,error:'Nicht autorisiert.'},401,origin);let b;try{b=await request.json()}catch{return reply({ok:false,error:'Ungültige Anfrage.'},400,origin)}try{const p=normalizeProduct(b),id='p_'+crypto.randomUUID().replaceAll('-','').slice(0,12);await env.DB.prepare('INSERT INTO products (product_id,sku,name,description,category,price_cents,variants_json,tags_json,personalization_label,personalization_placeholder,stock,unlimited_stock,active,sort_order) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)').bind(id,p.sku||null,p.name,p.description,p.category,p.price,JSON.stringify(p.variants),JSON.stringify(p.tags),p.label,p.placeholder,p.stock,p.unlimited,p.active,p.sort).run();return reply({ok:true,id},201,origin)}catch(e){return reply({ok:false,error:e.message||'Artikel konnte nicht angelegt werden.'},400,origin)}}
async function patchAdminProduct(request,env,origin,id){const a=await authenticateAdmin(request,env);if(!a.ok)return reply({ok:false,error:'Nicht autorisiert.'},401,origin);let b;try{b=await request.json()}catch{return reply({ok:false,error:'Ungültige Anfrage.'},400,origin)}try{const p=normalizeProduct(b);await env.DB.prepare('UPDATE products SET sku=?,name=?,description=?,category=?,price_cents=?,variants_json=?,tags_json=?,personalization_label=?,personalization_placeholder=?,stock=?,unlimited_stock=?,active=?,sort_order=?,updated_at=CURRENT_TIMESTAMP WHERE product_id=?').bind(p.sku||null,p.name,p.description,p.category,p.price,JSON.stringify(p.variants),JSON.stringify(p.tags),p.label,p.placeholder,p.stock,p.unlimited,p.active,p.sort,id).run();return reply({ok:true,id},200,origin)}catch(e){return reply({ok:false,error:e.message||'Artikel konnte nicht gespeichert werden.'},400,origin)}}
